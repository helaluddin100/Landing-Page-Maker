...
export default function AdminSectionsManagement() {
  ...
  const loadSections = async () => {
    try {
      ...
      if (filters.landing_page_id) queryParams.append('landing_page_id', filters.landing_page_id)
      if (filters.type) queryParams.append('type', filters.type)
      if (filters.is_active !== '') queryParams.append('is_active', filters.is_active)
      
      const response = await fetch(`/api/admin/sections?${queryParams.toString()}`)
      const result = await response.json()
      
      if (result.success) {
        setSections(result.data.data || result.data)
      }
    } catch (error) {
  ...
  const loadPages = async () => {
    try {
      ...
      const result = await response.json()
      if (result.success) {
        setPages(result.data.data || result.data)
      }
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        setMessage({ 
          type: 'success', 
          text: editingSection ? 'Section updated successfully!' : 'Section created successfully!' 
        })
        setShowModal(false)
      ...
      setMessage({ type: 'danger', text: error.message || 'Failed to save section' })
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleDelete = async (sectionId) => {
    if (confirm('Are you sure you want to delete this section?')) {
      try {
        ...
        setMessage({ type: 'danger', text: error.message || 'Failed to delete section' })
      }
    }
  }
  ...
  const handleDuplicate = async (sectionId) => {
    try {
      ...
      setMessage({ type: 'danger', text: error.message || 'Failed to duplicate section' })
    }
  }

  const toggleSectionStatus = async (section) => {
    try {
      const response = await fetch(`/api/admin/sections/${section.id}/toggle-status`, {
        method: 'POST'
      })
      ...
      if (result.success) {
        ...
        throw new Error(result.message || 'Failed to update section status')
      }
    } catch (error) {
      console.error('Error updating section status:', error)
      setMessage({ type: 'danger', text: error.message || 'Failed to update section status' })
    }
  }
          ...
          <Col>
            <div className="d-flex justify-content-between align-items-center mb-4">
              <div>
                ...
                <p className="text-muted">Create, edit, and manage sections for landing pages</p>
              </div>
              <Button 
                variant="primary" 
                ...
                <Row>
                  <Col md={4}>
                    <Form.Group>
                      ...
                      <Form.Select
                        value={filters.landing_page_id}
                        onChange={(e) => setFilters({...filters, landing_page_id: e.target.value})}
                      >
